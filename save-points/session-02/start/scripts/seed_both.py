...
async def _load_cosmos(
    ...
    # 힌트: CosmosClient → database → container 핸들을 얻고, corpus·embeddings 를
    # upsert_item({id, doc_id, title, content, embedding}) 으로 적재. 마지막에 client.close().
    raise NotImplementedError("_load_cosmos 를 구현하세요.")


async def _search_cosmos(
    credential: DefaultAzureCredential, query_emb: list[float]
) -> list[str]:
    # 힌트: VectorDistance(c.embedding, @e) ORDER BY 로 TOP_K doc_id 를 가져옵니다.
    raise NotImplementedError("_search_cosmos 를 구현하세요.")
...
async def _load_pg(
    ...
    # 힌트: 첫 연결은 register 없이 CREATE EXTENSION vector → 테이블(halfvec(3072)) →
    # HNSW 인덱스(halfvec_cosine_ops) 를 idempotent 하게 만든 뒤, register_vector_async 호출,
    # INSERT ... VALUES (..., HalfVector(emb)) ON CONFLICT DO UPDATE 로 적재.
    # (extension 생성 전 register 하면 chicken-and-egg 로 실패함에 주의)
    raise NotImplementedError("_load_pg 를 구현하세요.")


async def _search_pg(
    credential: DefaultAzureCredential, query_emb: list[float], ef_search: int
) -> list[str]:
    # 힌트: register_vector_async 후 트랜잭션 안에서 SET LOCAL hnsw.ef_search = ef_search,
    # embedding <=> %s ORDER BY 로 TOP_K doc_id 검색 (HalfVector(query_emb)).
    raise NotImplementedError("_search_pg 를 구현하세요.")
