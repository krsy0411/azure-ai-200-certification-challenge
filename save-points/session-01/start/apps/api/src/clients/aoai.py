...
def build_aoai_client(settings: Settings) -> AsyncAzureOpenAI:
    ...
    """
    # DefaultAzureCredential 생성하기

    # cognitiveservices.azure.com 스코프의 토큰 프로바이더 만들기

    # AsyncAzureOpenAI 클라이언트 만들어 반환하기
    # 힌트: azure_endpoint, azure_ad_token_provider, api_version 세 가지 인자가 필요합니다.
    raise NotImplementedError("Azure OpenAI 클라이언트 생성 로직을 채워 넣으세요.")
...
async def embed_text(client: AsyncAzureOpenAI, settings: Settings, text: str) -> list[float]:
    ...
    """
    # client.embeddings.create 호출해 임베딩 벡터 반환하기
    # 힌트: model = settings.azure_openai_embed_deployment, input = text
    raise NotImplementedError("임베딩 생성 호출을 채워 넣으세요.")
...
async def chat_with_context(
    ...
    """
    # 시스템 프롬프트 — "컨텍스트만 사용해 한국어로 간결하게 답변하세요" 같은 RAG 지시문
