...
# 관측성 — Azure Monitor + OpenTelemetry. app 생성 직후(요청 처리 전, 모듈 로드
# 시점)에 계측해야 인입 요청 server span(requests 테이블)이 잡힌다. lifespan startup
# 에서 호출하면 미들웨어 스택이 이미 만들어진 뒤라 server span 이 누락된다.
