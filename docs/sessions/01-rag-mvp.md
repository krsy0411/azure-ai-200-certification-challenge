...
### 1.7 (선택) 사용자 계정에도 Cosmos data plane · Key Vault Secrets 부여

`// -------- 6) (선택) 사용자에게도 Cosmos data plane · Key Vault Secrets 부여 모듈 호출하기` 힌트 주석을 찾아 다음 두 모듈로 교체합니다. `if (!empty(userObjectId))` 조건부 배포라, 배포 명령에 `userObjectId` 를 넘기지 않으면 건너뜁니다. 본인 `az login` 자격으로 Cosmos 시드 스크립트를 실행하고 Data Explorer 를 직접 확인하려면 이 블록을 채우고 배포 시 `userObjectId` 를 넘깁니다. 사용자 계정이므로 `principalType` 은 `User` 입니다.
...
- Azure Container Apps Container App 에 User Assigned Managed Identity 연결 ([1.8 Azure Container Apps — FastAPI (ca-api)](#18-azure-container-apps--fastapi-ca-api) 에서 진행)
...
프런트엔드도 두 곳에 빈칸이 있습니다. 분량이 길어 전체는 완성본을 참조하고, 여기서는 핵심만 짚습니다.

- `apps/web/app/api/chat/route.ts` — 브라우저 요청을 ca-api 의 `/api/chat` 으로 프록시하는 Next.js API Route. `API_BASE_URL` 환경변수 (서버 사이드 전용) 를 읽어 본문 검증 후 upstream 호출 결과를 그대로 전달합니다
- `apps/web/app/page.tsx` — 질문 입력 폼과 답변 · 출처 표시 UI. `handleSubmit` 안의 `fetch("/api/chat", ...)` 호출 한 곳을 채웁니다

핵심 호출부 (`page.tsx` 의 `handleSubmit` 안) 는 다음과 같습니다.
...
> [!TIP]
> 프런트엔드 두 파일의 전체 정답은 `save-points/session-01/complete/apps/web/app/page.tsx` 와 `.../app/api/chat/route.ts` 에 있습니다. 막혔다면 [1.11 조립 검증 — 컴파일](#111-조립-검증--컴파일) 의 완성본 덮어쓰기 명령으로 전체를 한 번에 맞춰볼 수 있습니다.
