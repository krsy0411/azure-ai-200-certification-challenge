...
#### 본 챌린지에서의 사용 방식 — Key Vault + Managed Identity
...
Key Vault + Managed Identity를 사용하는 방식이 코드로 그대로 표현되는 모듈입니다.
...
1. **Cosmos DB account** → **Data Explorer** → `appdb` → `chunks` 컨테이너 → **Items** 에 시드된 chunk (id · content · embedding 필드) 확인
...
2. **Azure Container Apps `ca-api-ai200ws-dev`** → **Log stream** → 방금 `curl` 한 요청이 FastAPI 로그에 실시간으로 확인
...
4. **Application Insights** → **Transaction search** → 한 요청 안에(HTTP 인입 → Cosmos query → Azure OpenAI 호출) span 트리 확인
...
5. **Key Vault** → **Access control (IAM)** → User Assigned Managed Identity 가 `Key Vault Secrets User` 역할 보유 확인
   ...
   **Role assignments(역할 할당)** 목록에서 User Assigned Managed Identity 가 `Key Vault Secrets User` 역할을 보유하는지 확인합니다.
