...
```bash
# Linux · macOS · WSL — DSv5 는 koreacentral 기본 vCPU 할당량이 0 이므로 DSv3 사용. 최소 4 vCPU 가용 확인
az vm list-usage --location koreacentral -o table | grep -E "DSv3"
```

```powershell
# Windows PowerShell — grep 대신 Select-String
az vm list-usage --location koreacentral -o table | Select-String -Pattern "DSv3"
```
...
```powershell
# Windows PowerShell
az bicep build --file infra/sessions/07-aks/main.bicep --outfile "$env:TEMP\main.json"
if ($?) { "BUILD OK" }
```
...
Invoke-RestMethod -Method Post -Uri "http://$LB_IP/api/chat" `
  -ContentType "application/json" `
  -Body '{"q":"휴가 규정 알려줘"}' | ConvertTo-Json
```
