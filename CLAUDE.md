...
이 레포는 **Microsoft Azure AI-200 자격증**의 핵심 스택을 다루는 RAG 지식 비서를 **참가자가 문서만 보고 본인 구독에 직접 올리는 "Workshop in a Day"** 산출물입니다. Claude Code에게 주는 컨텍스트를 여기에 모읍니다.

## 프로젝트 성격

- **워크샵 자료(참가자가 따라 하는 문서·코드·IaC)** 가 1차 산출물. 동시에 학습 + 포트폴리오 + 자격증 대비도 목표.
- 모든 변경은 **§11 "워크샵 참가자 관점 우선"** 을 기준으로 한다 — 개인적으로 Claude 와 함께 실행하는 맥락이 아니라, 처음 보는 참가자가 다양한 OS·환경에서 문서를 따라 할 때 동작하는지로 판단.
...
7. **자원 라이프사이클 — Phase 단위 정리 + *무료/사실상-무료 자원만 보존*.** 비용 통제와 학습 격리를 위해 Phase 마다 자원이 살았다 정리되는 사이클을 갖는다.
   - **비용 현실 (Phase 7 실측, 7일 108,569 KRW, docs/learning-paths/07-backend-services.md 함정 8)**: dev 환경이라도 idle 자원이 *시간당* 누적된다. Redis Enterprise Memory_M10 = ~11,680 KRW/일, ACA Container App (min replica 1) = ~1,743 KRW/일, AKS LB+IP = ~1,125 KRW/일, PG B1ms = ~700 KRW/일. **compute 가 있는 자원은 idle 만으로도 빠르게 누적**.
   ...
   - **Phase 종료 시 정리 + 다음 Phase 진입 시 재배포**: 사용자 명시 요청을 받으면 해당 Phase 의 *모든 compute/유료 자원* 을 정리한다. 다음 Phase 의 `main.bicep` 이 그 자원을 `existing` 참조한다면 다음 Phase 진입 직전에 해당 Phase 의 `main.bicep` 을 한 번 다시 돌려 재배포한다 (Phase 4 ↔ Phase 5 패턴). 같은 이름의 자원이 soft-delete 충돌하면 접미사를 한 단계 올린다 (`dev04` → `dev06`).
   - **보존 / 정리 분류 (Phase 7 실측 기반, 2026-05-18 룰 갱신)**:
     - **보존 (무료 또는 사실상-무료)**:
       ...
       - **Key Vault / App Configuration** (Phase 8 이후) — sub-원 단위 비용
     - **정리 (compute 또는 유의미한 idle 비용)**:
       - **Cosmos DB / Azure OpenAI / PostgreSQL / Managed Redis** — Phase 4·5·6 데이터 자원
       - **Service Bus / Event Grid / Function App / Storage** — Phase 7 신규
       - **ACA Container Apps (api / web)** — Phase 단위 정리, 다음 Phase 진입 시 image tag 와 함께 재배포 (CAE 는 보존이므로 컨테이너만 정리)
       - **AKS 클러스터 + 자동 생성된 `MC_...` RG** — Phase 3 학습 산출물이지만 LB+IP idle 비용. 학습 완료 후 정리. Phase 10 (포트폴리오 부스터) 에서 단순 manifest 시연이면 재배포
       - Phase 1 의 App Service / App Service Plan
   ...
   - **이전 룰 (2026-05-18 이전) 과의 차이**: 이전 룰은 *공용 UAMI / CAE / AKS / ACA api·web* 도 보존이었다. 실측 비용 (Phase 7) 으로 ACA + AKS 가 *진짜* idle 비용 발생 자원임이 드러나 보존 목록에서 제외. UAMI / CAE / LAW / ACR / ACR / AI 만 무료 또는 사실상 무료.
...
11. **워크샵 참가자 관점 우선 (workshop-participant-first) — 모든 문서·코드 변경의 기본 시점.** 이 레포는 *참가자가 문서만 보고 본인 구독·본인 컴퓨터에서 따라 하는 워크샵* 산출물이다. 무언가를 수정·반영할 때는 **"나(사용자)와 Claude 가 이 세션에서 함께 실행하는 개인적 맥락"이 아니라, 처음 보는 참가자가 문서를 따라 할 때 동작하는가** 를 기준으로 결정한다.
