...
3. **commit convention 준수** — `<type>(<scope>): <메시지>` 형식. 예: `feat(phase-6): Azure Managed Redis + 시맨틱 캐시 + chat.py RAG 화`
4. **메시지 본문은 한국어** — 접두사 (`feat`, `fix`, `docs`, `chore`, `refactor` 등) 와 scope (`phase-N`, `rules`, `hooks` 등) 만 영문, 콜론 뒤 본문은 한국어로 작성.
...
|---|---|---|---|
| 1 | **Phase-specific 산출물 (IaC)** | `infra/modules/<phase 자원>-*.bicep`, `infra/phases/0N-*/main.bicep`, `infra/phases/0N-*/main.bicepparam` | `feat(phase-N): ...` 또는 `feat(infra,phase-N): ...` |
| 2 | **Phase-specific 산출물 (앱 코드)** | `apps/api/src/**/*.py`, `apps/api/pyproject.toml` (Phase N 의존성 추가), `apps/web/...` | `feat(phase-N): ...` |
| 3 | **Phase 문서** | `docs/learning-paths/0N-*.md` | `docs(phase-N): ...` |
| 4 | **함정 fix (Phase 진행 중 발견)** | 이미 commit 된 Phase 코드에 대한 hotfix | `fix(phase-N): ...` |
| 5 | **레포 룰 / 워크플로우** | `CLAUDE.md`, `.claude/skills/**`, `.claude/agents/**`, `.claude/hooks/**`, `.claude/settings*.json` | `docs(rules): ...`, `feat(workflow): ...`, `feat(hooks): ...` |
| 6 | **history.md** | `docs/history.md` | `docs(history): ...` 단독 commit (다른 묶음과 합치지 않음) |
| 7 | **아키텍처 / 로드맵 / 공통 docs** | `docs/architecture.md`, `docs/roadmap.md`, `README.md` 등 | `docs(arch): ...`, `docs(roadmap): ...` |
| 8 | **CI / 빌드 설정** | `.github/workflows/**`, `Dockerfile`, `uv.lock` (단독 변경 시) | `chore(ci): ...`, `chore(build): ...` |
| 9 | **그 외 잡다한 변경** | 위에 안 잡힌 것 | `chore: ...` (scope 없어도 OK) |

**같은 묶음 안에서도 더 쪼개야 할 때**:
- 한 Phase 에 *기능 추가 (feat)* + *함정 fix (fix)* 가 섞이면 → type 별로 분할
- IaC + 앱 코드가 동시에 한 Phase 에서 변경됐고 양이 크면 → `infra,phase-N` / `phase-N` 으로 분할 (예: Phase 5 의 `feat(phase-5): PG ...` 와 `feat(phase-5): api 0.5.1 ...` 가 분리된 패턴 참고)
...
|---|---|---|---|
| 1 | feat(phase-6) | Azure Managed Redis + 시맨틱 캐시 + chat.py RAG 화 | infra/modules/redis-*.bicep, infra/phases/06-*/, apps/api/src/cache/, apps/api/src/messaging/, apps/api/src/routers/chat.py, apps/api/src/main.py, apps/api/pyproject.toml |
| 2 | docs(phase-6) | 학습 경로 정독 + 결정 + 측정 + 함정 5개 정식 기록 | docs/learning-paths/06-managed-redis.md |
| 3 | feat(commit-skill) | /commit Skill — 변경 사항을 의미 단위로 분할 commit | .claude/skills/commit/SKILL.md |
| 4 | docs(history) | Phase 6 종료·정리 완료 반영 | docs/history.md |
```
...
```
[1/4] 8a3f2c1 feat(phase-6): Azure Managed Redis + 시맨틱 캐시 + chat.py RAG 화
```
