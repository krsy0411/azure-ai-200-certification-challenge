# 챌린지 문서 스타일 가이드 (docs-writer 전용)

본 문서는 학습자가 읽는 자료가 아니라 **챌린지 문서 작성 시 따라야 하는 규칙** 입니다. 다음 문서가 본 가이드의 적용 대상입니다.
...
본 챌린지 자료는 **오프라인 행사 전용이 아닙니다**. 온라인에서 학습자가 혼자 자율 학습할 때도 동일하게 사용됩니다.

따라서 다음 가정을 **절대 깔지 않습니다**.

- 강사가 있다 (X)
- 챌린지 당일이라는 특정 날짜가 있다 (X)
- 시작일이 정해져 있다 (X)
- 학습자 전원이 같은 시각에 진행한다 (X)
- 오프라인 공유 키·공유 자원이 있다 (X)

> [!NOTE]
> 사용자 문서·산문에서는 항상 **"챌린지" / "Challenge"** 를 쓰고 **"워크샵" / "Workshop" 은 쓰지 않습니다** (§2.11). 본 가이드 본문에서도 같은 규칙을 적용하되, §6 체크리스트의 grep 패턴 문자열 (`워크샵 당일` 등) 은 검출용이므로 예외입니다.
...
분석에서 얻은 **반면교사** — ko-kr Learn 의 기계 번역 (MT) 페이지에는 `Container Apps**를 선택합니다**` (볼드 경계 깨짐), `중요합니다` (Alert 제목의 어색한 직역), `사용하십시오` 같은 번역체 결함이 있습니다. 본 챌린지 문서는 ko-kr Learn 의 **구조** (단계 구성·표·스크린샷 배치) 는 차용하되, **문장** 은 번역체가 아닌 자연스러운 한국어 (§2 규칙) 로 작성합니다.
...
- `~합니다` / `~합니다.`
- `~다` / `~다.`
- 서술어 없이 명사구로 종결 (예: `Log Analytics Workspace — 챌린지 전체의 중앙 로그`)
...
**볼드 안에 중첩된 기울임체** 는 마크다운에서 볼드를 다시 중첩할 수 없으므로 기울임 표시만 제거합니다. 이미 볼드 범위 안이면 해당 부분을 평문으로 둡니다.

| 금지 | 권장 |
|---|---|
| `**기대 출력 + *X*.**` | `**기대 출력 + X.**` |
| `**A 와 *B* 를 비교**` | `**A 와 B 를 비교**` |
...
처음 등장하는 자리에서 풀어쓰고, 같은 문단·섹션 내 반복 시에도 가능하면 풀어쓴 표현을 유지합니다. **본 규칙은 Azure 자원 약어** (AOAI · RG · ACA 등) **에만 적용합니다.**
...
**예외 (풀어쓰지 않고 그대로 유지하는 고유명)** — 풀어쓰면 오히려 비표준·어색해지는 다음 약어·고유명은 §2.5 의 적용 대상이 아닙니다.

| 유지 (풀어쓰지 않음) | 사유 |
|---|---|
| RAG | 개념 고유명 (Retrieval-Augmented Generation 으로 매번 풀면 산문이 무거워짐) |
| KQL | SQL 류 쿼리 언어명 (Kusto Query Language 보다 KQL 이 표준 표기) |
| `pgvector` · `halfvec` | PostgreSQL 확장·타입 고유명 |
| `RediSearch` | Redis 모듈 제품명 |
| `Flex Consumption` | Azure Functions 호스팅 플랜 제품명 |

> [!NOTE]
> 위 예외는 §6 의 약어 grep (#3) 에서도 검출 대상이 아닙니다. 해당 grep 주석에 RAG·KQL 예외를 명시해 둡니다.
...
>
> - `ingestion` → **인제스션** — 데이터를 받아들이는 행위 (예: Blob → Cosmos · PostgreSQL 적재). 본 챌린지의 비동기 인제스션 파이프라인이 여기에 해당합니다
...
`S00 자원의 사전 배포는 학습 가치를 위해 금지합니다` 같이 **행사 운영자의 시각** 에서 쓰는 문장은 사용하지 않습니다. 학습자는 본인 페이스로 진행하므로 "사전 배포 금지" 같은 통제는 의미가 없습니다.

### 2.11 용어 일관성 — "챌린지" 로 통일

사용자 문서·산문에서는 항상 **"챌린지" / "Challenge"** 를 사용하고 **"워크샵" / "Workshop" 은 사용하지 않습니다**.
...
| 이 워크샵에서는 | 이 챌린지에서는 |
| 워크샵 홈 | 챌린지 홈 |
| Workshop in a Day | (산문에서는 풀어 쓰거나 "챌린지" 로 표현) |

> [!IMPORTANT]
> **예외 — 코드·경로의 영문 `workshop/`** — save-point 작업 폴더명인 `workshop/` (예: `cp -a save-points/session-NN/start/. workshop/`) 은 실제 디렉터리 이름이므로 그대로 둡니다. 산문 표현만 "챌린지" 로 바꿉니다.

> [!NOTE]
> **문법 주의** — "챌린지" 는 받침이 없는 단어입니다. 조사는 받침 없는 형태를 씁니다.
>
> - `챌린지는` / `챌린지가` / `챌린지를` (○)
> - `챌린지은` / `챌린지이` / `챌린지을` (✗)
...
모든 세션 문서는 **제목 → breadcrumb → 사전 준비 조건 Alert** 순으로 시작합니다.
...
- 제목은 `# session-NN (<제목>)` **괄호 형식**. 과거의 `# session-NN — <제목>` em-dash 형식은 사용하지 않습니다
- 제목 바로 다음 줄에 `👈 [챌린지 홈](../../README.md)` breadcrumb 을 둡니다 (§3.6 — breadcrumb 은 문서 상단, 푸터는 별도)
- 이어서 `> [!IMPORTANT]` GitHub Alert. 헤더는 `**사전 준비 조건**`. 조건은 불릿 사용
- `사전 조건` 금지 → 항상 `사전 준비 조건`

> [!IMPORTANT]
> **과거의 `> **관련 Microsoft Learn 학습 경로**` 콜아웃 블록은 더 이상 넣지 않습니다.** 학습 경로 매핑은 README 가 담당합니다 (세션 문서에 학습 경로 링크를 중복으로 두지 않습니다).
...
> [!IMPORTANT]
> **세션 문서에 두지 않는 헤더** — 다음 두 헤더는 더 이상 세션 문서에 두지 않습니다.
>
> - `## 0. 이 세션에서 경험하는 내용` — README 로드맵 표의 "본인 손으로 경험하는 것" 열이 이 역할을 대체합니다 (§3.3)
> - `## 주의` 요약 섹션 — 함정·주의의 단일 Source of Truth 는 `docs/pitfalls/common.md` 입니다 (§3.12)
> - `## 참고 자료` — 외부 링크는 README 학습경로 매핑·본문 인라인에서 처리합니다 (§3.11 하단)

### 3.3 `## 0. 이 세션에서 경험하는 내용` — 더 이상 사용하지 않음

과거 세션 문서 도입부에 두던 `## 0. 이 세션에서 경험하는 내용` 섹션은 **더 이상 사용하지 않습니다**.

- **사유**: README 로드맵 표의 "본인 손으로 경험하는 것" 열이 이 역할을 대체합니다. 세션마다 같은 내용을 중복으로 두지 않습니다
- 세션 문서는 §3.1 헤더 메타 블록 (제목 → breadcrumb → 사전 준비 조건) 직후 바로 `## 시작본 코드 받기` (§3.10) 또는 `## 1단계 · …` 로 이어집니다
- 세션이 챌린지 전체에서 차지하는 맥락을 짧게 알려야 한다면, 별도 인트로 섹션 대신 첫 단계 도입 문장에 녹입니다
...
### 3.6 세션 네비게이션 (breadcrumb + 푸터)

네비게이션은 **상단 breadcrumb** 과 **하단 푸터** 두 곳으로 나뉩니다.

**상단 breadcrumb** — §3.1 대로 제목 바로 다음 줄에 챌린지 홈 링크만 둡니다.
...
**하단 푸터** — 모든 세션 문서의 **마지막 줄** 에 좌우 화살표 손가락 이모지와 함께 이전·다음 세션 링크를 둡니다. 라벨은 `[session-NN]` 으로 **축약** 합니다 (긴 부제는 붙이지 않습니다).

```markdown
---

👈 [session-N-1](./N-1-slug.md) | [session-N+1](./N+1-slug.md) 👉
```

규칙.

- 첫 세션 (`session-00`) 푸터는 다음 링크만 둡니다 — `[session-01](./01-rag-mvp.md) 👉`
- 중간 세션 푸터 — `👈 [session-N-1](…) | [session-N+1](…) 👉`
- 마지막 세션 (`session-07`) 푸터 — `👈 [session-06](./06-observability.md) | [자원 정리](../cleanup.md) 👉`
- 가운데 구분자는 ` | ` (앞뒤 공백 1칸씩)
- 좌측 이모지는 `👈`, 우측 이모지는 `👉`
- 푸터 링크 텍스트는 `session-NN` 축약 형태 (세션 제목 부제 금지)

> [!IMPORTANT]
> 과거의 `[← 워크샵 홈]` 푸터 링크 규칙은 사용하지 않습니다. 챌린지 홈으로 가는 링크는 **상단 breadcrumb** 한 곳에만 둡니다.
...
세션 docs 의 마지막 `## 마무리` 섹션 직전에 다음 안내를 둡니다 (`## 주의` 요약 섹션은 더 이상 두지 않습니다 — §3.12).
...
> [!IMPORTANT]
> `git tag session-NN-start` 또는 `git checkout session-NN-start` 같은 git 기반 save-point 표현은 사용하지 않습니다. 본 챌린지의 save-point 메커니즘은 **단일 main 브랜치 + 폴더 복사** 입니다 ([save-points/README.md](../../save-points/README.md) 참고).

### 3.11 디렉터리 트리 표기

모듈·파일 목록을 보여줄 때는 bullet 나열 대신 ` ```text ` 코드펜스의 **디렉터리 트리** 로 보여줍니다. 루트 경로 한 줄 + `├──` / `└──` 가지 + 파일명 옆 정렬된 `#` 역할 주석으로 구성합니다.
...
- 첫 줄은 트리의 루트 경로 (`apps/api/src/`) 한 줄
- 가지 기호는 `├──` (중간 항목) · `└──` (마지막 항목) · `│   ` (들여쓰기 연결선)
- 파일명과 `#` 주석 사이는 공백으로 **세로 정렬**
- 주석은 §2.1 종결 규칙대로 명사구로 끝냅니다

> [!NOTE]
> **`## 참고 자료` 섹션 미사용** — 세션 문서에는 `## 참고 자료` 섹션을 두지 않습니다. 외부 링크는 README 학습경로 매핑과 본문 인라인 링크로 처리합니다.

### 3.12 주의·함정 정책

함정·주의 정보의 위치를 다음 네 규칙으로 통일합니다.

1. **세션 문서 끝의 `## 주의` 요약 섹션을 두지 않습니다.** (§3.2 — 헤더 명명에서도 제거)
2. **함정·주의의 단일 Source of Truth 는 `docs/pitfalls/common.md`** 입니다 — 카테고리·세션 태그·증상/원인/회피를 한곳에 모읍니다
3. **본문 인라인 함정 callout 은 유지합니다** — 해당 단계에서 just-in-time 으로 필요한 `> [!WARNING]` / `> [!CAUTION]` 은 흐름상 그대로 둡니다 (§3.5 배치 규칙)
4. **README 의 "진행 중 막혔을 때" 섹션이 `common.md` 로 가는 단일 진입점** 입니다 — 세션마다 common.md 링크를 흩뿌리지 않습니다
...
학습자가 챌린지 문서에서 코드를 **그대로 복사·붙여넣기** 해서 동작시키는 것이 목표입니다. 따라서:
...
**placeholder 표기 보강 (MicrosoftLearning lab 대비 차별점)** — MicrosoftLearning lab 은 `<your-resource-group-name>` 같은 꺾쇠 placeholder 를 쓰지만, 꺾쇠 placeholder 는 그대로 복사하면 명령이 실패하므로 본 챌린지에서는 사용하지 않습니다. 항상 `$VAR` + 변수 채우는 선행 명령 패턴을 사용합니다.
...
### 4.4 bash 블록은 명령 단위로 분할

주석으로 구분된 여러 명령을 하나의 ` ```bash ` 펜스에 몰아넣지 않습니다. **명령 (또는 단일 목표) 단위로 개별 ` ```bash ` 블록** 으로 나눕니다.

- 각 블록 앞에는 §4.3 도입 문장을 둡니다 (그 블록이 무엇을 하는지)
- **예외 — 환경변수로 연결된 파이프라인** (빌드 → 푸시 → 배포 → 호출 등 앞 명령의 출력·변수를 뒤 명령이 그대로 쓰는 흐름) 은 한 블록으로 유지합니다 (§4.1 의 `OID=$(…)` → 배포 예시처럼)
- **점검용 명령 나열은 `&&` 체인으로 묶지 않습니다** — 하나가 실패해도 다음 점검이 이어지도록 개별 블록으로 둡니다

| 금지 | 권장 |
|---|---|
| 무관한 명령 3개를 한 펜스에 `# A` … `# B` … `# C` 로 몰아넣기 | A·B·C 를 각각 도입 문장 + 개별 ` ```bash ` 블록으로 분리 |
| `az ... && docker ... && curl ...` 점검 체인 | 점검마다 개별 블록 (하나 실패해도 나머지 진행) |

### 4.5 크로스플랫폼 — OS 별 명령 차이 병기

학습자가 다양한 OS 에서 따라 하므로, OS 마다 다른 명령은 차이를 병기합니다 (참여자 재현성).

- **`python` 명령은 OS 마다 다릅니다** — macOS·Linux 는 `python3`, Windows 는 `python` 또는 `py`. 점검·명령에는 OS 주석을 병기합니다
- 점검 나열을 `&&` 로 묶어 한쪽 OS 에서 전체가 중단되지 않게 합니다 (§4.4 와 동일 원칙)
- 셸 차이가 있는 명령은 §3.10 처럼 `# Linux · macOS · WSL` / `# Windows PowerShell` 주석으로 분기합니다
...
> [!NOTE]
> 본 §5 의 일부 규칙은 외부 자료인 microsoft/moaw 의 스크린샷 배치 패턴을 차용했습니다. 차용 출처를 가리킬 때 등장하는 `moaw` 는 외부 자료 고유명이며, 본 챌린지 산문에 적용되는 §2.11 용어 규칙과는 별개입니다.
...
- **행동 직후 결과 확인용으로만** 삽입합니다 — Portal 폼 작성을 안내한 직후, 배포 완료 화면, 데이터가 들어간 것을 확인하는 화면 등. moaw 의 "절차 지시 → 결과 스크린샷" 배치를 따릅니다
...
- **캡션** 은 이미지 바로 아래 1~2문장. 단순 화면 설명이 아니라 **학습자가 그 화면에서 무엇을 확인해야 하는지** (확인 포인트·기대 값) 를 적습니다. moaw 는 캡션을 거의 쓰지 않지만, 본 챌린지는 자율 학습용이므로 확인 포인트 캡션을 필수로 둡니다
...
문서 출고 (챌린지 공개) 전에는 이 grep 이 빈 결과여야 합니다. 작성 단계에서는 남아 있는 것이 정상입니다.
...
# 3. 약어 잔재 (RAG·KQL 및 pgvector/halfvec/RediSearch/Flex Consumption 은 §2.5 예외 — 검출 대상 아님)
...
# 14. 한국어 "워크샵" 잔재 (영문 workshop/ 경로는 제외 — §2.11)
grep -n '워크샵' <파일>

# 15. ## 0 인트로 섹션 잔존 (§3.3 — 더 이상 사용 안 함)
grep -nE '^## 0\.' <파일>

# 16. ## 참고 자료 섹션 잔존 (§3.11 — 미사용)
grep -n '^## 참고 자료' <파일>

# 17. ## 주의 요약 섹션 잔존 (§3.12 — common.md 로 일원화)
grep -nE '^## 주의' <파일>

# 18. em-dash 제목 (§3.1 — 괄호 형식으로 통일)
grep -nE '^# session-[0-9]+ —' <파일>

# 19. breadcrumb 존재 점검 (역방향 — 세션 문서당 정확히 1개)
grep -c '^👈 \[챌린지 홈\]' <파일>
```

12번 (작성 단계) 과 19번 (역방향 — 결과가 `1` 이어야 함) 을 제외하고 모두 빈 결과여야 합니다.
